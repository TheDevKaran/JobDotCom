...
import 'package:jobdotcom/models/response/auth/login_res_model.dart';
...
class AuthHelper {
  ...
  static Future<bool> updateProfile(ProfileUpdateReq model) async {
    ...
    String token = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2NTQyNDFiZmY4NTJiMmRjNTc4YjA2Y2YiLCJlbWFpbCI6ImRldmthcmFueWFkYXZtaWxsaUBnbWFpbC5jb20iLCJ1c2VybmFtZSI6InZlZCIsImlzQWRtaW4iOnRydWUsImlhdCI6MTY5OTQ3NzQ3Nn0.tnSg9GaLx68fyCBz9z0R-Md0AO4NA7Se3tx4TmlCxkI";
    Map<String, String> requestHeaders = {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer $token',
    };
    var url=Uri.https(Config.apiUrl, Config.profileUrl);
    // var userId = int.parse(Config.userId);
    // var url = Uri.https(Config.apiUrl, '${Config.profileUrl}/$userId');
    // var url = Uri.https(Config.apiUrl, '${Config.profileUrl}/${Config.userId}');
    var response = await client.put(url, headers: requestHeaders,body: jsonEncode(model)
